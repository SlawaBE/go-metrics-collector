...
func TestJsonBatchUpdateMetricsHandler_ServeHTTP(t *testing.T) {
	type input struct {
		method string
		json   string
	}
	...
	tests := []struct {
		...
		},
		{
			...
			data: input{
				method: http.MethodPost,
				json:   "[{\"id\":\"name\",\"type\":\"counter\",\"delta\":\"a\"}]",
			},
			want: output{
				statusCode: 400,
			},
		...
		{
			name: "Success metrics update",
			data: input{
				method: http.MethodPost,
				json:   "[{\"id\":\"name\",\"type\":\"counter\",\"delta\":1},{\"id\":\"name\",\"type\":\"gauge\",\"value\":1.1}]",
			},
			want: output{
				statusCode: 200,
			},
